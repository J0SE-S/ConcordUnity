...
namespace Dissonance.Networking.Server.Admin
{
        ...
        where TPeer : struct, IEquatable<TPeer>
    {
        ...
        public void InvokeOnClientEnteredRoom([NotNull] ClientInfo<TPeer> peer, string name)
        {
            ...
            _knownRoomNames[new RoomName(name, true).ToRoomId()] = name;
        }
        ...
        public void InvokeOnRelayingPacket(ArraySegment<byte> payload, TPeer source)
        {
            ...
            if (!reader.ReadPacketHeader(out var type))
            {
            ...
            // Read out the set of channels for this player, this will also detect spoofing of voice packets
            ReadChannels(clientState, reader);
            clientState.InvokeOnVoicePacket(reader);
        }
