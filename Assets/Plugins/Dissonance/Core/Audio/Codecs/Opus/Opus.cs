        ...
        public static int SampleRate(this OpusNative.Bandwidth bandwidth)
        {
            switch (bandwidth)
            {
                ...
                case OpusNative.Bandwidth.Fullband:
                    return 48000;
                default:
                    throw new ArgumentOutOfRangeException(nameof(bandwidth), Log.PossibleBugMessage(string.Format("{0} is not a valid value", bandwidth), "B534C9B2-6A9B-455E-875E-A01D93B278C8"));
            }
        }
    }
...
#region fields and properties
            ...
            public float PacketLoss
            {
                ...
                {
                    if (value < 0 || value > 1)
                        throw new ArgumentOutOfRangeException(nameof(value), Log.PossibleBugMessage(string.Format("Packet loss percentage must be 0 <= {0} <= 1", value), "CFDF590D-C61A-4BB4-BB2D-1FAC1E59C114"));
