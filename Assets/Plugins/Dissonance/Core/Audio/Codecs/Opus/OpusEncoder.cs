        ...
        : IVoiceEncoder
    {
        ...
        public ArraySegment<byte> Encode(ArraySegment<float> samples, ArraySegment<byte> encodedBuffer)
        {
            if (Array.IndexOf(PermittedFrameSizesSamples, samples.Count) == -1)
                throw new ArgumentException(Log.PossibleBugMessage(string.Format("Incorrect frame size '{0}'", samples.Count), "6AFD9ADF-1D15-4197-99E9-5A19ECB8CD20"), nameof(samples));
